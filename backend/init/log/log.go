...
import (
	...
	log2 "github.com/Fighting2520/panelLearn/backend/log"
...
func setOutput(logger *logrus.Logger, config configs.LogConfig) {
	writer, err := log2.NewWriterFromConfig(&log2.Config{
...
func (s *MineFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	detailInfo := ""
	if entry.Caller != nil {
		function := strings.ReplaceAll(entry.Caller.Function, "github.com/Fighting2520/panelLearn/cmd/server/backend/", "")
